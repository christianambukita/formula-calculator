...
function isLastNumberIntiger(array){
    let symbolList = [...array].reverse().filter(element => typeof element === STRING_TYPE);
...
function operatorSubReducer(stateBase, state, operator, lastElement){
    ...
    if((lastElement === OPERATORS.DOT || lastElement === OPERATORS.MINUS) && typeof state[state.length-2] == STRING_TYPE){
    ...
    switch(operator){
        ...
        case OPERATORS.DOT: 
            if(newLastElement === OPERATORS.DOT || !isLastNumberIntiger(oldState)) return oldState
            return [...oldState, operator]
        default:
    ...
    switch(typeof key){
        case NUMBER_TYPE:
            if(typeof lastElement === NUMBER_TYPE){
                if(lastElement === 0){
                    if(typeof state[lastIndex-1] === NUMBER_TYPE) return [...state, key]   // 5 0 +key = 5 0 key  not 5 key
            ...
            return [...state, key]
        case STRING_TYPE:
            return operatorSubReducer([...newStateBase], [...state], key, lastElement);
        default:
            return state;
    }
}
