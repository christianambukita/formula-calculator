...
function TopButtonBar({ handleButtonClick, handleSpecialSymbol }) {
	return (
		<div id='top-button-bar'>
			...
			{listToButtons(
				['CE'],
				() => handleSpecialSymbol(SPECIAL_SYMBOLS.CE),
				'button top-btn',
				'tbb-'
			)}
			{listToButtons(
				['C'],
				() => handleSpecialSymbol(SPECIAL_SYMBOLS.C),
				'button top-btn',
				'tbb-'
			)}
			{listToButtons(
				['<-'],
				() => handleSpecialSymbol(SPECIAL_SYMBOLS.REMOVE),
				'button top-btn',
				'tbb-'
			)}
		</div>
...
export default connect(null, (dispatch) => ({
	...
	handleSpecialSymbol: (symbol) => dispatch({ type: symbol }),
}))(TopButtonBar);
